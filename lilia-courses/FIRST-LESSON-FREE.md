...
### 2. Env for default course and redirect

- **`PAYMENT_COURSE_ID`** – UUID of the “default” course (e.g. Relatia 360). Used by `/acces-curs` to know which first lesson to open after signup, and by sync-entitlements after payment. **Required** for the signup→lesson redirect to work.
- **`FIRST_LESSON_ID`** – (recomandat) UUID-ul primei lecții. După signup, userul e dus la `/app/lesson/[acest ID]`. Obții ID-ul din Supabase: `SELECT get_first_lesson_id('ID-CURSULUI');` → pui rezultatul în Vercel ca `FIRST_LESSON_ID`.
- For other courses use `/acces-curs?course=UUID`.

### 3. Env for syncing entitlements after payment
...
1. User goes to **/acces-curs** (or `/acces-curs?course=UUID` for another course). One form: **email**, **phone**, **password** → account created. If email confirmation is off in Supabase, they are logged in and redirected **directly to the first lesson** of that course. The course is determined by `?course=` or by `PAYMENT_COURSE_ID` (default Relatia 360).
